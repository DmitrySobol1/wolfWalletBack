    ...
    for (const item of recordsNew) {
      ...
      if (item.status == 'new') {
        ...
        if (payStatus[0].status.toLowerCase() == 'finished') {
          ...
          if (!ourComissionResponse) {
            throw new Error(
              'не получен ответ от функции getOurComissionMarket'
            );
          }
          ...
          //создать запрос на вывод - уже есть токен в начале кода
          ...
          if (verify === 'OK') {
            ...
            if (!modelOperation) {
              throw new Error(
                'не прошли изменения в бд RqstStockMarketOrderModel'
              );
            }
          }
      ...
      if (item.status == 'CoinReceivedByStock') {
        ...
        if (placeOrderFunction.status === 'ok') {
          ...
          if (!modelResponse) {
            throw new Error(
              'не записаны изменения в бд RqstStockMarketOrderModel'
            );
          }
      ...
      if (item.status == 'orderPlaced') {
        ...
        const modelResp = await RqstStockMarketOrderModel.findOneAndUpdate(
          { _id: item._id },
          {
            $set: {
        ...
        if (!modelResp) {
          throw new Error('инфо не обновилось в бд RqstStockMarketOrderModel');
        }
      ...
      if (item.status == 'stockTrtFromTradeToMain') {
        ...
        const getNpAdressResult = createPayAdress(
          token,
          coinToSendToNpFull,
          amountToSendToNp,
          item.userNP
        ...
        if (!modelResp) {
          throw new Error('не записалосб в бд RqstStockMarketOrderModel');
        }
        ...
        const modelResp2 = await RqstStockMarketOrderModel.findOneAndUpdate(
          { _id: item._id },
          {
            $set: {
        ...
        if (!modelResp2) {
          throw new Error('не записалось в бд RqstStockMarketOrderModel');
        }
      ...
      if (item.status == 'stockSentCoinToNp') {
        ...
        if (payStatusFunction.result == 'ok') {
          ...
          if (
            ...
            const modelResp3 = await RqstStockMarketOrderModel.findOneAndUpdate(
              { _id: item._id },
              {
                $set: {
                  status: 'done',
